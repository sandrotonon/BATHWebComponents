\chapter*{Anhang B - Polymer Web Komponente}\label{anhang-b---polymer-web-komponente}
\addcontentsline{toc}{chapter}{Anhang B - Polymer Web Komponente}
...
\CommentTok{<!--}
\CommentTok{@license}
\CommentTok{Copyright (c) 2015 The Polymer Project Authors. All rights reserved.}
\CommentTok{This code may only be used under the BSD style license found at http://polymer.github.io/LICENSE.txt}
\CommentTok{The complete set of authors may be found at http://polymer.github.io/AUTHORS.txt}
\CommentTok{The complete set of contributors may be found at http://polymer.github.io/CONTRIBUTORS.txt}
\CommentTok{Code distributed by Google as part of the polymer project is also}
\CommentTok{subject to an additional IP rights grant found at http://polymer.github.io/PATENTS.txt}
\CommentTok{-->}
      ...
      \DataTypeTok{author}\OperatorTok{:} \OperatorTok{\{}
        ...
        \DataTypeTok{value}\OperatorTok{:} \KeywordTok{function}\NormalTok{() }\OperatorTok{\{}
          \ControlFlowTok{return} \OperatorTok{\{}
            ...
            \DataTypeTok{image}\OperatorTok{:} \StringTok{'http://addyosmani.com/blog/wp-content/uploads/2013/04/unicorn.jpg'}\OperatorTok{,}
          \OperatorTok{\};}
        \OperatorTok{\}}
      \OperatorTok{\},}
    ...
    \KeywordTok{<paper-drawer-panel>}
      ...
      \KeywordTok{<paper-header-panel}\OtherTok{ main}\KeywordTok{>}
        \KeywordTok{<paper-toolbar>}
          \KeywordTok{<paper-icon-button}\OtherTok{ icon=}\StringTok{"menu"}\OtherTok{ paper-drawer-toggle}\KeywordTok{></paper-icon-button>}
...
\KeywordTok{<script>}
  \AttributeTok{Polymer}\NormalTok{(}\OperatorTok{\{}
    ...
    \DataTypeTok{_getSelectedContent}\OperatorTok{:} \KeywordTok{function}\NormalTok{() }\OperatorTok{\{}
      \ControlFlowTok{return} \KeywordTok{this}\NormalTok{.}\VariableTok{$}\NormalTok{.}\VariableTok{leftnavi}\NormalTok{.}\AttributeTok{getContentChildren}\NormalTok{()[}\KeywordTok{this}\NormalTok{.}\VariableTok{$}\NormalTok{.}\VariableTok{topnavi}\NormalTok{.}\AttributeTok{selected}\NormalTok{].}\VariableTok{selectedItem}\NormalTok{.}\AttributeTok{linkNum}\OperatorTok{;}
    \OperatorTok{\},}

    \DataTypeTok{_addSelectedHandler}\OperatorTok{:} \KeywordTok{function}\NormalTok{() }\OperatorTok{\{}
      ...
      \VariableTok{topNaviItems}\NormalTok{.}\AttributeTok{addEventListener}\NormalTok{(}\StringTok{'iron-select'}\OperatorTok{,} \KeywordTok{function}\NormalTok{() }\OperatorTok{\{}
        ...
        \KeywordTok{var} \NormalTok{leftSelected }\OperatorTok{=} \VariableTok{leftNaviItems}\NormalTok{.}\AttributeTok{getContentChildren}\NormalTok{()[}\VariableTok{that}\NormalTok{.}\AttributeTok{selectedTop}\NormalTok{].}\AttributeTok{selectedItem}\OperatorTok{;}
...
\emph{Diese Komponente kann auch auf GitHub unter
https://github.com/glur4k/multi-navigation-app und in dem
customelements.io Elemente Katalog unter
https://customelements.io/glur4k/multi-navigation-app/. Sie kann
ebenfalls mit Bower \texttt{bower\ install\ multi-navigation-app}
