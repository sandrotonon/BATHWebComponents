...
Statt neue Elemente zu erzeugen können sowohl native HTML-Elemente als bereits erstellte Custom Elements um Funktionen und Eigenschaften erweitert werden, auch "Type Extensions" genannt. Diese erben von einem spezifischen HTMLElement, also "Element X ist ein Y". Zusätzlich zum Namen des erweiterten Elements wird nun der Prototyp, sowie der Name des zu erweiternden Elements der `registerElement`-Funktion als Parameter übergeben. Soll nun ein erweitertes `button`-Element erzeugt werden, muss folgendes gemacht werden:
